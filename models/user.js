...
try {
  db.exec("ALTER TABLE fc_users ADD COLUMN is_admin INTEGER DEFAULT 0;");
} catch (e) {}
...
const db_ops = {
  ...
  create_admin: db.prepare(
    "INSERT INTO fc_users (username, passhash, created_at, is_admin) VALUES (?, ?, ?, 1) RETURNING user_id AS id;",
  ),
  get_user: db.prepare(
    "SELECT user_id AS id, username, attributes, created_at, is_admin FROM fc_users WHERE id = ?;",
  ),
  find_by_username: db.prepare(
    "SELECT user_id AS id, username, attributes, created_at, is_admin FROM fc_users WHERE username = ?;",
  ),
  ...
  promote_to_admin: db.prepare(
    `UPDATE fc_users SET is_admin = 1 WHERE user_id = ?;`,
  ),
};
...
export function getUser(userId) {
  let { id, username, attributes, created_at, is_admin } = db_ops.get_user.get(userId);
  return {
    id,
    username,
    created_at,
...
export function promoteToAdmin(userId) {
  db_ops.promote_to_admin.run(userId);
}
...
export async function createAdmin(username, password) {
  ...
  let createdAt = Date.now();
  let passhash = await argon2.hash(password, HASH_PARAMS);
  return db_ops.create_admin.get(username, passhash, createdAt);
}
